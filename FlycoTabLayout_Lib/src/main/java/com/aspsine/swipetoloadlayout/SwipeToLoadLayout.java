...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    private OnDepthRefreshReleaseListener mDepthRefreshReleaseListener;
    ...
    private View mPulldownVisibleView;
    ...
    //设置下拉跳转新页面开关
    private boolean depthRefreshEnable;

    //是否当次下拉属于应用开启后自动下拉
    boolean isAutoPullToDepth;

    public boolean isDepthRefreshEnable() {
        return depthRefreshEnable;
    }

    public void setDepthRefreshEnable(boolean depthRefreshEnable) {
        this.depthRefreshEnable = depthRefreshEnable;
        if (mHeaderView != null && mHeaderView instanceof SwipeRefreshHeaderLayout) {
            SwipeRefreshHeaderLayout view = (SwipeRefreshHeaderLayout) mHeaderView;
            view.setDepthRefreshEnable(depthRefreshEnable);
        }
    }
    ...
    /**
     * <b>ATTRIBUTE:</b>
     * depth offset to trigger refresh
     */
    private float mRefreshDepthTriggerOffset;
    /**
     * <b>ATTRIBUTE:</b>
    ...
    public SwipeToLoadLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                } else if (attr == R.styleable.SwipeToLoadLayout_refresh_depth_trigger_offset) {
                    setRefreshDepthTriggerOffset(a.getDimensionPixelOffset(attr, 0));
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mPulldownVisibleView = findViewById(R.id.swipe_pull_down_view);
        } else {
            // more than four children: unsupported!
        ...
        if (mPulldownVisibleView != null) {
            mPulldownVisibleView.setVisibility(View.GONE);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            ...
            if (mRefreshDepthTriggerOffset <= mHeaderHeight) {
                mRefreshDepthTriggerOffset = (float) (1.5 * mHeaderHeight);
            }
        }
        // target
        if (mTargetView != null) {
            ...
            if (lp != null) {
                mTargetViewHeight = targetView.getMeasuredHeight() + lp.topMargin + lp.bottomMargin;
            } else {
                mTargetViewHeight = 1920;
            }
        }

        //pull down visible view
        if (mPulldownVisibleView != null) {
            final View pullDownView = mPulldownVisibleView;
            measureChildWithMargins(pullDownView, widthMeasureSpec, 0, heightMeasureSpec, 0);
        }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (STATUS.isRefreshStatus(mStatus)) {
                    if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isReleaseToRefresh(mStatus)) {
                        ...
                        float marginBottom = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 120, getContext().getResources().getDisplayMetrics());
                        if ((!depthRefreshEnable && mTargetOffset >= screenHeight / 2) || mTargetOffset >= (screenHeight - marginBottom)) {
                        } else {
                            fingerScroll(yDiff);
                        }
                    }
    ...
    /**
     * set the value of {@link #mRefreshDepthTriggerOffset}.
     * Default value is the double refresh header view height {@link #mHeaderHeight}<p/>
     * If the offset you set is smaller than {@link #mHeaderHeight} or not set,
     * using {double @link #mHeaderHeight} as default value
     *
     * @param offset
     */
    public void setRefreshDepthTriggerOffset(int offset) {
        mRefreshDepthTriggerOffset = offset;
    }
    ...
    /**
     * set an{@link OnDepthRefreshReleaseListener} to listening depth release
     *
     * @param mDepthRefreshReleaseListener
     */
    public void setOnDepthRefreshReleaseListener(OnDepthRefreshReleaseListener mDepthRefreshReleaseListener) {
        this.mDepthRefreshReleaseListener = mDepthRefreshReleaseListener;
        if (mDepthRefreshReleaseListener != null && mPulldownVisibleView != null) {
            mPulldownVisibleView.setVisibility(View.VISIBLE);
        }
    }
    ...
    public void setAutoPullToDepth() {
        if (!isRefreshEnabled() || mHeaderView == null) {
            return;
        }
        isAutoPullToDepth = true;
        setStatus(STATUS.STATUS_SWIPING_TO_REFRESH);
        scrollReleaseToDepth();
    }
    ...
    private void layoutChildren() {
        ...
        // layout pull down view
        if (mPulldownVisibleView != null) {
            final View pulldownVisibleView = mPulldownVisibleView;
            MarginLayoutParams lp = (MarginLayoutParams) pulldownVisibleView.getLayoutParams();
            final int headerLeft = paddingLeft + lp.leftMargin;
            final int headerTop;
            switch (mStyle) {
                case STYLE.CLASSIC:
            ...
            final int headerRight = headerLeft + pulldownVisibleView.getMeasuredWidth();
            final int headerBottom = headerTop + pulldownVisibleView.getMeasuredHeight();
            pulldownVisibleView.layout(headerLeft, headerTop, headerRight, headerBottom);
        }
    ...
    private void onActivePointerUp() {
        if (STATUS.isSwipingToRefresh(mStatus)) {
            ...
            if (depthRefreshEnable && mPulldownVisibleView != null && mHeaderOffset >= mRefreshDepthTriggerOffset && mDepthRefreshReleaseListener != null) {
                mDepthRefreshReleaseListener.startDepthRefresh();
                scrollReleaseToDepth();
            } else {
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                if (depthRefreshEnable && mPulldownVisibleView != null && (mHeaderOffset > mRefreshDepthTriggerOffset || isAutoPullToDepth) && mDepthRefreshReleaseListener != null) {
                    Log.d("--------------", "mheaderOffset=" + mHeaderOffset);
                    isAutoPullToDepth = false;
                    mDepthRefreshReleaseListener.depthRefreshRelease(mPulldownVisibleView);
                } else {
                    finish();
                }
            } else {
